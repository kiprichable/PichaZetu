...
use App\Notifications\SendCustomerEmail;
...
class ActivationRepository
{
    ...
    public function createTokenAndSendCustomerEmail(User $user)
    {
        $activations = Activation::where('user_id', $user->id)
            ->where('created_at', '>=', Carbon::now()->subHours(config('settings.timePeriod')))
            ->count();
        ...
        // Send activation email notification
        self::sendNewCustomerEmail($user, $activation->token);
    }
	...
	public function sendNewCustomerEmail(User $user, $token)
	{
		$user->notify(new SendCustomerEmail($token));
	}
